...
function validateWords(rawWords) {
    ...
    return WORD_SPECS.map((spec) => {
        ...
        if (!source)
            throw new Error(`Nedostaje riječ ${spec.id}.`);
        const value = String(source.value || '')
            .toUpperCase()
            .replace(/[^A-ZČĆŽŠĐ]/g, '')
            .trim();
        if (value.length !== spec.length)
            throw new Error(`Riječ ${spec.id} mora imati ${spec.length} slova.`);
...
function pushMessage(room, text, kind = 'info') {
...
function getRoom(code) {
    ...
    if (!room)
        throw new Error('Soba ne postoji.');
    return room;
}
function getPlayer(room, playerId, token) {
    const player = room.players.find((item) => item.id === playerId && item.token === token);
    if (!player)
        throw new Error('Nevažeći pristup sobi.');
    return player;
}
...
function refreshBattleStatus(room) {
...
function revealCellForBoth(attacker, defender, cell, hitKind) {
    attacker.enemyGrid[cell] = hitKind;
    defender.defenseGrid[cell] = hitKind;
}
function checkWin(room, defender, attacker) {
...
app.post('/api/rooms', (req, res) => {
    try {
        ...
        const player = {
            ...
            moves: 0,
            words: [],
            defenseGrid: makeGrid('empty'),
            enemyGrid: makeGrid('unknown'),
        };
        const room = {
            code: roomCode(),
            players: [player],
...
app.post('/api/rooms/:code/setup', (req, res) => {
    try {
        ...
        player.score = 0;
        player.moves = 0;
        player.enemyGrid = makeGrid('unknown');
        player.defenseGrid = makeGrid('empty');
...
app.post('/api/rooms/:code/chat', (req, res) => {
    try {
        ...
        if (!text)
            throw new Error('Poruka je prazna.');
        if (text.length > 180)
            throw new Error('Poruka je predugačka.');
        room.chat.push({
            id: uid(),
            playerId: player.id,
...
app.post('/api/rooms/:code/guess', (req, res) => {
    try {
        ...
        if (!word)
            throw new Error('Riječ nije pronađena.');
        if (word.guessed)
            throw new Error('Ta riječ je već riješena.');
        attacker.moves += 1;
        if (guess !== word.value) {
            room.turn = defender.id;
